...
export const PersistedWorkflowNodeStateSchema = z.object({
  ...
  clarificationItems: z.array(z.object({
    id: z.string(),
    title: z.string(),
    gap: z.string(),
    impact: z.enum(['高', '中', '低']),
    ...
    status: z.enum(['UNANSWERED', 'ANSWERED', 'FROM_SOURCE', 'UNCONFIRMABLE', 'USED_IN_REVISION']),
  })).optional(),
  clarificationHistory: z.array(z.object({
    version: z.number().int().positive(),
    projectUnderstandingRevision: z.number().int().positive(),
  ...
  clarificationVersion: z.number().int().positive().optional(),
