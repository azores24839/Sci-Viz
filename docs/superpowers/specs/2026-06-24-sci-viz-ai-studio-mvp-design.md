...
> 日期：2026-06-24  
> 状态：已批准；2026-06-25 修订为“静图拍摄策划 + 案例库对标”方向
> 2026-07-04 更新：默认工作流、02 节点边界、Case Hub 使用方式与联网资料收集已由 `2026-07-04-sci-ai-studio-shooting-strategy-workflow-design.md` 修订。冲突处以新文档为准；本文件保留早期 MVP 架构与历史决策。
> 首个示例：长兴海洋实验室｜拍摄静图  
> 产品形态：阶段式 AI 工作台

## 1. 产品结论

科研影像 AI Studio 面向需要为科研团队制定静态拍摄方案的视觉策划与摄影团队。它不是科研项目知识管理工具，也不是普通 AI 文案生成器，而是把上传资料、已有照片和 Sci-Viz Case Hub 案例库转化为可解释、可对标、可沟通、可执行的拍摄静图方案。

MVP 只把“拍摄静图”做深，完整闭环为：

```text
混合资料与已有照片进入
  → AI 诊断团队当前视觉状态
  → 用户确认主目标、次目标和产物类型
  → AI 从案例库中选择同类型/同方向/同目标静图参照
  → AI 形成科研视觉策展策略
  → AI 生成对外沟通版策划提案与摄影师执行版清单
  → 可选生成 AI 参考图作为沟通草稿
```

未来支持科研视频，但首版只开放“拍摄静图”。“录影/影片”在新建项目与目标配置中显示为暂不可选，数据模型保留 `projectType`、`outputType` 与可扩展产物类型，避免后续破坏式迁移。
...
- 科研视觉策划、摄影与宣传团队
- 对科研内容并不天然熟悉，但需要在有限时间内判断“现有视觉哪里不足、应该参考谁、下一次应该怎么拍”
- 负责创建项目、整理材料、选择传播目标、调整策划、组织静图拍摄与交付参考方案

### 2.2 协作者

- 科研人员、项目负责人或实验室宣传接口人
- 不需要注册完整账号
- 通过带有效期的审核链接确认科学事实、保密边界、安全要求、可拍条件和关键表达

### 2.3 MVP 成功标准

- 摄影团队能在 15–30 分钟内掌握一个科研团队的视觉现状、短板和可拍机会
- 用户能明确选择一个主目标和最多一个次目标，避免方案同时服务过多目的
- 系统能从案例库中给出同类型、同科研方向或同传播目标的静图参考，并解释为什么参考
- 每条关键科研结论、保密风险和拍摄条件可以追溯到资料来源，或被明确标记为待确认
- 科研人员能在 10 分钟内完成一次轻量确认，重点确认事实、风险与现场可拍条件
- 已确认内容能直接转化为对外沟通版策划提案和摄影师执行版清单
- 画面卡能清楚说明拍什么、为什么拍、怎么拍，以及对应的参考案例或 AI 参考图
...
2. 拍摄静图项目类型；数据库预留视频类型，UI 中“录影/影片”显示暂不可选
3. 混合资料入口：文件、网页链接、文本；MVP 使用预置 Mock 资料
4. 资料卡片、已有照片缩略图、解析状态、来源信息和异常状态
5. 视觉现状诊断：团队类型、科研方向、已有素材结构、视觉短板、可拍机会
6. 目标与产物选择：主目标 1 个，次目标 0–1 个；拍摄静图可选，录影/影片暂不可选
7. 案例库对标：同类型/同科研方向/同传播目标静图案例、推荐理由和降级匹配说明
8. 科研策展策略：视觉路线、画面气质、核心表达、参考理由和传播重点
9. 贯穿式科学审校结果：矛盾、风险、待确认问题、保密、安全和可拍条件提示
10. 无需注册的科研人员审核页：确认、修改、留言
11. 对外沟通版策划提案：现状诊断、目标、对标案例、视觉路线、拍摄主题和风险
12. 摄影师执行版清单：拍摄对象、场景、人物/设备/材料、景别、角度、光线、色调、动作、优先级和注意事项
13. 画面卡：表达目的、拍摄对象、景别、机位、构图、光线、动作、科学信息、参考案例图/AI 参考图、优先级
14. 可选 AI 参考图预览：用于沟通的概念参考图，不替代真实拍摄
15. 执行清单：未拍、已拍、需补拍、备注、文件编号、临时新增
16. 四个可见 Agent 的角色、头像占位、职责、状态、输入、输出和运行记录
17. 黑、白、`#2569ED` 设计系统
18. 版本记录、审计记录和确定性的 Mock Agent 工作流
19. 基于 React Flow 的固定业务流程画布、节点状态、节点图片和右侧 Agent 上下文面板
20. 响应式桌面工作台；执行清单页面兼容手机宽度

### 3.2 明确不做

- 真实论文/PDF/PPT 内容解析
- 真实大模型调用与计费
- 真实 AI 生图服务接入；MVP 只显示 Fixture/Mock 参考图或生成占位
- 原始照片上传、选片、修图和后期交付
- PDF 导出
- 视频脚本、采访提纲、时间码、视频分镜和录影执行清单
- 完整团队账号、组织权限和商业计费
- 多 Agent 自由讨论或聊天表演
...
```text
资料输入 → 视觉现状诊断 → 目标与产物选择 → 案例对标 → 策展策略 → 静图拍摄方案 → AI 参考图 → 方案输出
```

| 稳定节点 ID | 显示名称 | 类型 | 负责人 |
| --- | --- | --- | --- |
| `source-intake` | 资料输入 | 人工/系统 | 资料管理员 |
| `visual-diagnosis` | 视觉现状诊断 | Agent | 资料分析师 |
| `goal-output-selection` | 目标与产物选择 | 人工闸门 | 项目负责人 |
| `case-benchmark` | 案例对标 | Agent | 科研策展人 |
| `curation-strategy` | 策展策略 | Agent + 人工确认 | 科研策展人 |
| `photo-plan` | 静图拍摄方案 | Agent + 人工确认 | 摄影策划师 |
| `ai-reference` | AI 参考图 | Agent/系统 | 摄影策划师 |
| `plan-output` | 方案输出 | 系统产物 | 项目负责人 |

`plan-output` 在 MVP 中输出可查看、可版本化的网页方案，不承诺 PDF 导出。显示名称可以调整，稳定节点 ID 一经发布不得复用为其他语义。

节点卡只显示足以判断进度的摘要、代表图片和关键标签。长内容、编辑器、证据、版本差异和产物预览放在右侧面板或专用详情页，避免把画布变成拥挤表单。

节点卡视觉规则：

| 节点 | 卡片视觉 |
| --- | --- |
| `source-intake` | 上传资料、已有照片、官网截图缩略图 |
| `visual-diagnosis` | 现有素材拼贴、缺口标签 |
| `goal-output-selection` | 主目标、次目标、静图模式标识 |
| `case-benchmark` | Sci-Viz Case Hub 代表案例缩略图 |
| `curation-strategy` | mood strip、视觉路线关键词 |
| `photo-plan` | 画面卡缩略图 |
| `ai-reference` | AI 参考图预览或生成占位 |
| `plan-output` | 策划提案封面与执行清单摘要 |
...
- 资料概览：文件、网页、文字材料、已有照片和解析状态
- 团队类型：高校课题组、实验室、平台型机构、企业研发中心等
- 科研语境：研究方向、成果形态、人物、设备、空间、实验对象
- 已有视觉资产结构：人物、会议、设备、过程、成果、环境等占比与示例
- 视觉短板：过度会议化、缺少过程、缺少尺度、缺少细节、缺少应用场景等
- 可拍机会：设备、过程、样品、人物协作、空间、工程场景
- 来源抽屉：来源页面、原文片段、访问日期、可信状态

完成门槛：资料已就绪，视觉现状诊断已生成，阻塞级事实/保密/安全问题已列出。

#### 阶段二：确定目标与对标

页面区块：

- 主目标：学术传播、公众传播、招生/招聘/团队品牌、产业转化/合作，必须选 1 个
- 次目标：同一组选项，可选 0–1 个
- 产物类型：拍摄静图可选；录影/影片显示暂不可选
- 对标筛选：团队类型、科研方向、传播目标、静图媒介
- 案例推荐：代表图、来源、三轴标签、推荐理由
- 降级匹配说明：同方向不足时如何退到同团队类型、同目标或跨领域优秀案例

完成门槛：主目标与产物类型已确认，至少 3 个对标案例进入策展策略。
...
- 视觉路线：从当前状态走向哪种表达
- 对标案例拆解：借鉴点、不能照搬点、适配理由
- 策展 brief：主目标、次目标、受众、核心表达、画面气质
- 科学与现场风险：待确认事实、保密边界、安全距离、禁拍内容
- 拍摄主题与叙事结构
- 核心传播信息
- 场景、人物和设备安排
- 画面卡网格
...
完成门槛：策展策略已确认，静图拍摄方案已生成，至少一张必拍画面卡存在，项目负责人标记“可执行”。

#### 阶段四：方案输出与执行

页面区块：

- 对外沟通版策划提案：现状诊断、目标、案例对标、视觉路线、拍摄主题、参考图和风险
- 摄影师执行版清单：拍摄对象、场景、人物/设备/材料、景别、角度、光线、色调、动作、优先级
- 按场景分组的拍摄清单
- 每项状态：未拍、已拍、需补拍
- 文件编号与现场备注
- 临时新增画面
...
| `/projects/new` | 新建项目 | 首版只开放拍摄静图；录影/影片暂不可选 |
| `/projects/:id/studio` | AI Studio | 节点画布与右侧上下文面板 |
| `/projects/:id/diagnosis` | 视觉现状诊断 | 可深链访问的资料、现状诊断和可拍机会 |
| `/projects/:id/benchmark` | 案例对标 | Case Hub 参考案例、筛选依据和推荐理由 |
| `/projects/:id/strategy` | 策展策略 | 视觉路线、策展 brief、风险和确认事项 |
| `/projects/:id/plan` | 静图拍摄方案 | 策划提案、画面卡、AI 参考图和执行清单 |
| `/projects/:id/capture` | 执行清单 | 独立且可移动端使用的清单 |
...
| Agent | 负责 | 不负责 |
| --- | --- | --- |
| 资料分析师 | 读取上传资料、已有照片和网页信息，判断团队类型、科研语境、视觉短板和可拍机会 | 选择最终传播目标、决定视觉路线、生成完整拍摄方案 |
| 科研审校员 | 贯穿检查科学事实、保密边界、安全要求、可公开内容和可拍条件，生成待确认问题 | 替科研人员做最终确认、把缺失事实写成确定事实 |
| 科研策展人 | 结合目标和 Sci-Viz Case Hub 案例库，选择对标案例、解释参考理由、提出视觉路线和策展 brief | 具体机位参数、现场执行清单、科学事实裁决 |
| 摄影策划师 | 把策展 brief 转化为静图拍摄方案、画面卡、AI 参考图提示和摄影师执行清单 | 改写已确认的科学事实、生成视频脚本或时间码 |

### 5.2 角色边界原则

- “资料分析师”负责看清现有视觉状态；“科研策展人”负责把案例库研究转化为视觉策略；“摄影策划师”负责把策略转成可拍画面
- “科研审校员”不是主流程中的重型关卡，而是贯穿式风险层：在诊断、策展和拍摄方案中持续标注待确认、保密、安全和可拍条件
...
- 阻塞级科学、保密、安全或可拍条件问题未确认时，拍摄方案可以预览但不能标记为“可执行”
...
- `outputType`: `PHOTO_STATIC | VIDEO_UNAVAILABLE`
- `status`: `DRAFT | ACTIVE | COMPLETED | ARCHIVED`
- `description`
- `mockMode`
- `primaryGoal`: `ACADEMIC_COMMUNICATION | PUBLIC_COMMUNICATION | RECRUITING_BRAND | INDUSTRY_COLLABORATION`
- `secondaryGoal`: 同组选项，可为空，且不能等于 `primaryGoal`
...
- `projectId`
- `sourceDocumentId`
- `kind`: `EXISTING_PHOTO | SCREENSHOT | REFERENCE_IMAGE | AI_REFERENCE`
- `title`
- `storageKey`
- `thumbnailUrl`
- `sourceUrl`
- `visualCategory`: 人物、会议、设备、过程、成果、环境、样品、应用场景等
- `diagnosisTagsJson`
- `metadataJson`

用于表达用户已有照片、资料截图、案例参考图和 AI 参考图。已有照片进入视觉现状诊断，但不等同于最终交付照片。
...
- `projectId`
- `caseReferenceId`
- `provider`: `FIXTURE | CASE_HUB_HTTP`
- `title`
- `thumbnailUrl`
- `sourceName`
- `sourceDomain`
...
案例对标优先匹配“同团队类型 + 同科研方向 + 同传播目标 + 静图”。样本不足时允许降级，但必须记录降级原因，避免把泛泛好图包装成精确参照。

### 8.9 Artifact / ArtifactVersion

- `Artifact`: 项目、类型、当前版本
- `ArtifactVersion`: 版本号、结构化内容、生成来源、创建人、AgentRun、变更说明
- 产物类型：`VISUAL_DIAGNOSIS | SCIENCE_RISK_REVIEW | BENCHMARK_BRIEF | CURATION_STRATEGY | CLIENT_PROPOSAL | PHOTO_PLAN | SHOT_BOARD | EXECUTION_CHECKLIST | AI_REFERENCE_SET`
...
### 8.15 WorkflowTemplate / ProjectWorkflow

`WorkflowTemplate` 是版本化代码资产，不把可执行逻辑存成数据库脚本：

- `templateId`: `research-static-photo-v1`
- `version`
- `projectType`
...
### 8.16 WorkflowNodeState / CanvasLayout
...
PATCH  /api/v1/projects/:id/goals
...
GET    /api/v1/projects/:id/visual-assets
GET    /api/v1/projects/:id/benchmark-cases
POST   /api/v1/projects/:id/benchmark-cases/refresh
...
- UI 明确显示“Mock AI”
- AI 参考图在 MVP 中使用 Fixture 图片或占位图；真实生图服务以后通过同一 `ModelGateway` 或独立 `ImageGenerationGateway` 接入
...
- 从视觉诊断、目标选择、案例对标、策展策略到静图拍摄方案的工作流
- 重复任务幂等
- Agent 失败与重试
- 外部审核回复更新问题与事实状态
- 人工编辑后重新生成不会覆盖当前版本
- 案例对标产物能引用 FixtureCaseReferenceProvider，并记录推荐理由
- 对外沟通版策划提案和摄影师执行版清单可以分别读取
...
2. 验证八个业务节点、连线、MiniMap、缩放和画布锁定
3. 选择视觉现状诊断节点，确认右侧面板同步且业务状态未被改变
4. 查看四种 Mock 资料、已有照片缩略图、视觉现状诊断和一条事实的来源证据
5. 在目标与产物选择中选择一个主目标、一个次目标，确认录影/影片显示暂不可选
6. 进入案例对标，查看至少 3 个静图参考案例、推荐理由和降级说明
7. 运行或查看资料分析师、科研策展人、摄影策划师与科研审校员记录
8. 创建审核链接并进入外部审核页
9. 修正一个问题、确认一个问题并提交
10. 返回画布，确认节点阻塞数和状态变化
11. 进入静图拍摄方案，查看对外沟通版策划提案、摄影师执行版清单、AI 参考图和画面卡
12. 编辑一张画面卡并保留版本
13. 进入执行清单，完成必拍项、标记补拍项并添加备注
14. 刷新后验证业务状态与画布布局分别恢复
15. 在移动端宽度验证执行清单页面与线性流程降级
...
- 长兴示例包含资料、已有照片缩略图、视觉现状诊断、目标选择、案例对标、策展策略、待确认问题、审核回复、静图拍摄方案、AI 参考图、画面卡和执行清单
- 四个 Agent 的头像占位、角色和状态在 UI 中清晰可见
- 八个固定业务节点、连线、节点图片、MiniMap、缩放、画布锁定和选中态与参考图形成一致的工作区结构
- 新建项目和目标配置中“拍摄静图”可选，“录影/影片”显示暂不可选
- 用户必须选择一个主目标，可选择最多一个次目标
- 案例对标能显示静图参考案例、匹配依据、推荐理由和降级说明
- 用户能从画布完成主路径，不依赖手工修改数据库
- 画布布局与工作流状态独立持久化；刷新、布局重置和画布故障不改变业务状态
- 窄屏或画布加载失败时，核心流程可通过线性列表继续操作
- 外部审核链接可以完成科学事实、保密边界、安全要求和可拍条件的确认与修正
- 对外沟通版策划提案和摄影师执行版清单可以分别查看
- 画面卡可以编辑，执行状态可以持久化
...
2. 与案例库只读 API 联通
3. 真实 AI 生图服务接入，用于生成可追溯的拍摄参考图
4. 科研视频项目：叙事大纲、脚本、采访、分镜和镜头时长
5. 实拍素材上传、元数据、AI 覆盖检查和补拍建议
6. 团队账号、机构空间、权限与商业计费
