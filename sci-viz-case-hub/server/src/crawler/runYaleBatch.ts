...
const YALE_SOURCE_NAMES = [
  'Yale 主页',
  'YaleNews',
  'Yale School of the Environment',
  'Yale Engineering (SEAS)',
];
...
async function main() {
  ...
  console.log(`=== Yale Crawl Batch ===`);
  console.log(`Mode: ${execute ? 'EXECUTE' : 'DRY-RUN (add --execute to crawl)'}`);

  const sources = await prisma.crawlSource.findMany({
    where: { name: { in: YALE_SOURCE_NAMES }, enabled: true },
    orderBy: { id: 'asc' },
  });

  if (sources.length === 0) {
    console.error('No Yale sources found. Run seed script first.');
    process.exit(1);
  }
