...
async function main() {
  ...
  const sources = await prisma.crawlSource.findMany({
    where: { name: { in: ethSourceNames }, enabled: true },
    orderBy: { id: 'asc' },
  });

  if (sources.length === 0) {
    console.error('No ETH Zurich sources found. Run seed script first.');
    process.exit(1);
  }
