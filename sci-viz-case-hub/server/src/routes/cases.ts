...
async function buildWhere(query: Record<string, unknown>, exclude: string[] = []): Promise<Prisma.VisualCaseWhereInput> {
  ...
  if (!exclude.includes('visual_style') && query.visual_style) where.visualStyle = query.visual_style;
...
casesRouter.get('/cases/facet-counts', async (req: Request, res: Response) => {
  try {
    ...
    const [mediaTypeCounts, disciplineCounts, visualStyleCounts, contentTypeCounts, captureTypeCounts, sourceDomainCounts] = await Promise.all([
      ...
      prisma.visualCase.groupBy({ by: ['visualStyle'], where: await buildWhere(q, ['visual_style']), _count: { _all: true } }),
    ...
    const allSources = await prisma.crawlSource.findMany({ select: { name: true, url: true } });
    ...
    res.json({
      ...
      data: {
        ...
        visualStyle: toMap(visualStyleCounts, 'visualStyle'),
...
casesRouter.patch('/cases/:id', async (req: Request, res: Response) => {
  try {
    const allowedFields = [
      'title', 'mediaType', 'contentType', 'discipline', 'visualStyle',
